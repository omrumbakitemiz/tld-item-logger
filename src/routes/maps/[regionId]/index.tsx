...
export default component$(() => {
  ...
  const currentRegion = useSignal<Region | undefined>(() =>
    regions.find((region) => region.id === loc.params.regionId),
  );
  ...
  const drop = $(async (event: DragEvent) => {
    ...
    if (regionMapRef.value && currentDragItem.value && currentRegion.value) {
    ...
    <>
      <div class="flex justify-between px-6 py-4">
        <div>
          ...
          >
            {regions.map((region) => (
              <option key={region.id} value={region.id} selected={currentRegion.value?.id === region.id}>
                {region.name}
              </option>
            ))}
          </select>
              ...
              onClick$={async () => {
                if (confirm('Are you sure you want to clear the map?')) {
                  if (currentRegion.value) {
                    await deletePinsByRegionId(currentRegion.value.id);
                  }
                }
              }}
            >
